...
def compute_pbest(swarm):
    ...
    try:
        ...
        mask_cost = (swarm.current_cost < swarm.pbest_cost) {{{ ((swarm.current_cost == swarm.pbest_cost) and (np.count_nonzero(swarm.position) < np.count_nonzero(swarm.pbest_pos)))
