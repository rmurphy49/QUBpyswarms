...
def compute_pbest(swarm):
    ...
    try:
        ...
        mask_cost = np.logical_or(swarm.current_cost < swarm.pbest_cost, np.logical_and(swarm.current_cost == swarm.pbest_cost, np.count_nonzero(swarm.position) < np.count_nonzero(swarm.pbest_pos)))
...
def compute_velocity(swarm, clamp, vh, i, bounds=None):
    ...
    try:
        ...
        temp_velocity = (weight * swarm.velocity) + cognitive + social
        updated_velocity = vh(
